...
| 設定 | 型 | 既定 | 説明 |
|---|---|---|---|
| `window.width` | int(px) | 1100 | 新規ウィンドウの初期幅 |
| `window.height` | int(px) | 760 | 新規ウィンドウの初期高さ |
| `window.remember_position` | bool | true | 前回ウィンドウ位置を記憶 |
...
| ─ | | |
| Copy Path | 絶対パスをコピー | arboard |
| Copy Relative Path | プロジェクトルート基準の相対パス | arboard |
| ─ | | |
