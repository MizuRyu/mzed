...
対象: 設定画面の General セクション / 右クリックメニュー（サイドバーのファイル行・本文の選択範囲）/ メモ / キーバインド閲覧 / オーバーレイ一覧（プロジェクトメニュー・コマンドパレット）の選択操作。
...
本文を選択した状態で右クリックすると、項目1つのメニューを出す。

| 項目 | 動作 |
|---|---|
| メモを追加 | 選択範囲を引用してメモ入力欄を開く（→ 下記「メモ」） |

挙動メモ:
- **選択が無いときは介入しない**。WebView 標準のメニュー（コピー / 調べる）がそのまま出る。
- 介入するのは「右クリック位置が、引用元の選択と同じペイン本文の中」のときだけ。別ペインや余白を右クリックしたときは標準メニューに戻す。
- 選択の取得は WebView 側の `contextmenu` リスナー。ペイン本文（`.markdown-body[data-mdo-pane]`）の中の選択だけが対象で、Task View のプレビューやサイドバーの選択はメモにできない（どのファイルに紐づくか決まらないため）。
- 外側クリック / Escape で閉じる。
...
Cmd+F の検索バーと同じ位置・同じ見た目の小さな欄。上段に引用（1行に潰して 80 文字で省略）、下段に入力。
Enter で保存、Esc で閉じる。空文字は保存しない。検索バーとは同時に出さない（両方向で排他: メモを開くと Cmd+F の欄を閉じ、Cmd+F を押すとメモ欄を閉じる）。

### 選択の取得と紐づけ

選択の取得は WebView 側で行う。パレットや右クリックメニューは開いた時点でフォーカスを奪い document の選択を壊すため、
**ペイン本文で最後に確定した選択を WebView 側で覚えておく**。忘れる条件は2つ:

- 本文をクリックして選択を解いたとき（オーバーレイの入力欄へフォーカスが移っただけなら忘れない）
- 引用元のテキストノードが document から外れたとき（ライブリロード・タブ切替・テーマ切替による再描画、分割の解除）。覚えている引用が画面上の文章と食い違うのを防ぐ

紐づけ先のファイルは**選択を取得した時点**で確定する。メモ欄を開いた後にタブを切り替えても、保存されるのは選択したときのファイル（そのタブを閉じていても同じ）。
