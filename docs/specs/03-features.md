...
| N-02 | 目次 (ToC) パネル | ★★★ |
| N-03 | マルチタブ（全タブを同じ1行に並べて横スクロール。重ね描きはしないので他のタブに被らない。ホイール（縦ホイールも横に流す）とトラックパッドの横スクロールで送れる。スクロールバーは 4px。タブ切替時はアクティブタブを可視範囲へ寄せる。各タブは 96〜220px でファイル名は省略記号、同名ファイルが複数あるタブには親ディレクトリ名を薄く添える。ホバーでプロジェクト相対パスを表示。新規タブは既定で左端に入る（`tab_insert` で末尾に戻せる。既に開いているタブを開き直したときは位置を動かさない） → [10](10-settings-and-context-menu.md)） | ★★★ |
| N-04 | ファイル内検索 (Cmd+F) | ★★★ |
| N-05 | 全文検索 | ★★☆ |
